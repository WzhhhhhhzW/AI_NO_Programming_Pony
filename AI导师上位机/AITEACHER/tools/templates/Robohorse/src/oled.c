...
void Send_Byte(u8 dat)
{
    ...
    R_IIC_MASTER_SlaveAddressSet(&OLED_ctrl, 0x3C, I2C_MASTER_ADDR_MODE_7BIT);
    assert(FSP_SUCCESS == err);

    err = R_IIC_MASTER_Write(&OLED_ctrl, &dat, 1, true);
     assert(FSP_SUCCESS == err);
...
void OLED_WR_Byte(u8 dat,u8 mode)
{
    ...
    err = R_IIC_MASTER_Write(&OLED_ctrl, ii, 0x02, true);
    assert(FSP_SUCCESS == err);
    /* Since there is nothing else to do, block until Callback triggers*/
    //while ((I2C_MASTER_EVENT_TX_COMPLETE != i2c_event) && timeout_ms)
    while ((I2C_MASTER_EVENT_TX_COMPLETE != i2c_event) && timeout_ms>0)
...
void OLED_Refresh(void)
{
    ...
    for(i = 0; i < oled_y; i++)
    {
        ...
        // 发送整帧显示数据（129字节）
        err = R_IIC_MASTER_Write(&OLED_ctrl, buf, sizeof(buf), true);
        assert(FSP_SUCCESS == err);
...
void OLED_Show(double a0)
{
    int a1,a2,a3;
    a1=a0*10/100;
    a2=a0*10/10;
    a3=a0*10;
   OLED_ShowNum(9,9,a1*10+a2,2,16,1);
   OLED_ShowString(28, 9, ".", 16, 1);
   OLED_ShowNum(37,9,a3,1,16,1);
}
